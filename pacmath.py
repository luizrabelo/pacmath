...
def executar_jogo_pacmath():
    ...
    LARGURA_PIXELS = 1400  # largura da tela em pixels
    ALTURA_PIXELS = 500    # altura da tela em pixels
    LARGURA_CELULA = LARGURA_PIXELS // LARGURA_TABULEIRO  # largura de cada célula
    tela = pygame.display.set_mode((LARGURA_PIXELS, ALTURA_PIXELS))
    ...
    def tela_escolher_dificuldade():
        ...
        tela.blit(titulo, (LARGURA_PIXELS//2 - titulo.get_width()//2, 50))
        ...
        for i, opcao in enumerate(opcoes):
            ...
            retangulo = pygame.Rect(LARGURA_PIXELS//2 - 200, y, 400, 50)
    ...
    def desenhar_tela_jogo(mostrar_equacao=True, destacar_errado=False, valor_correto=None):
        ...
        pygame.draw.rect(tela, COR_PAREDE, (LARGURA_PIXELS - 10, POSICAO_Y_COMIDA - 40, 10, 80))
        ...
        tela.blit(nome2, (LARGURA_PIXELS-120, 20))
        ...
        tela.blit(acertos2, (LARGURA_PIXELS-220, 50))
        
        # Desenha a mensagem atual
        if mensagem:
            msg = fonte_pequena.render(mensagem, True, COR_MENSAGEM)
            tela.blit(msg, (LARGURA_PIXELS//2 - msg.get_width()//2, ALTURA_PIXELS-40))
        ...
        if mostrar_equacao:
            ...
            tela.blit(superficie_equacao, (LARGURA_PIXELS//2 - superficie_equacao.get_width()//2, ALTURA_PIXELS-150))
        ...
        for idx, alternativa in enumerate(alternativas):
            x_botao = LARGURA_PIXELS//2 - (len(alternativas) * largura_botao + (len(alternativas)-1) * espaco_botao) // 2 + idx * (largura_botao + espaco_botao)
            retangulo = pygame.Rect(x_botao, y_botao, largura_botao, altura_botao)
            botoes.append(retangulo)
            
            # Escolhe a cor do botão (verde se selecionado)
            cor = COR_BOTAO_SELECIONADO if alternativa_selecionada == idx else COR_BOTAO
    ...
    def desenhar_tela_fim_jogo():
        ...
        tela.blit(titulo, (LARGURA_PIXELS//2 - titulo.get_width()//2, ALTURA_PIXELS//2 - 100))
        ...
        tela.blit(msg_reiniciar, (LARGURA_PIXELS//2 - msg_reiniciar.get_width()//2, ALTURA_PIXELS//2 - 50))

    def nova_pergunta():
        """Gera uma nova pergunta de multiplicação"""
        nonlocal numero_a, numero_b, pergunta, resposta_correta, alternativas, alternativa_selecionada
    ...
    def animar_transicao_equacao():
        ...
        for _ in range(12):
            ...
            tela.fill(COR_FUNDO, (0, ALTURA_PIXELS-90, LARGURA_PIXELS, 90))
            tela.blit(superficie_equacao, (LARGURA_PIXELS//2 - superficie_equacao.get_width()//2, ALTURA_PIXELS-80))
            pygame.display.flip()
            pygame.time.delay(40)

    def processar_resposta(resposta):
        """Processa a resposta do jogador e atualiza o estado do jogo"""
        nonlocal jogador_atual, vencedor, estado_jogo, mensagem, posicao_bola, alternativa_selecionada
        ...
        if resposta == resposta_correta:
            ...
            mensagem = f"Errado! A resposta era {resposta_correta}. Tente novamente."
            acertos_consecutivos[jogador_atual] = 0
            # Não move o Pacman, mantém o mesmo jogador e mesma questão

    def reiniciar_jogo():
        """Reinicia o jogo para o estado inicial"""
        nonlocal posicao_bola, jogador_atual, acertos_consecutivos, estado_jogo, vencedor, texto_usuario, alternativa_selecionada
        posicao_bola = CENTRO
        jogador_atual = 0
